...
A WhatsApp bot built with Node.js and `whatsapp-web.js` to provide real-time cryptocurrency information, price tracking alerts, currency conversion, AI chat, and group management utilities.
...
- **Currency/Coin Converter (`!<amount>`, `!<amount> to`):** Convert between cryptocurrencies and fiat currencies based on real-time rates. Always includes the USD value. Powered by CoinGecko.
- **Generative AI Chat (`!metagpt`):** Start a persistent chat session with a Gemini AI model. The bot remembers your conversation history and custom identity until you stop the session.
...
- **Bot Information (`!meta`):** Displays information about the bot and a list of available commands.
...
- **Google AI (Gemini) API Key:** Get one for free from [Google AI Studio](https://aistudio.google.com/).
...
This bot uses an unofficial WhatsApp API (`whatsapp-web.js`). Using it carries a risk of your WhatsApp number being **permanently banned**. Use a dedicated, non-essential number for this bot. The creator is not responsible for any consequences of using this bot. Use at your own risk. Always Do Your Own Research (DYOR).